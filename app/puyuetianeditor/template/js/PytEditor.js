...
function PytCmd($p1, $p2, $p3) {
	if ($('#PytMainContent').css('display') == 'none') {
		...
		if (((PytEditor.selection && (navigator.userAgent.indexOf('MSIE') >= 0) && (navigator.userAgent.indexOf(
					'Opera') < 0)) ||
				(!!window.ActiveXObject || "ActiveXObject" in window)) && $p1 == 'inserthtml') {
...
function PytInsertImage() {
	...
	for (var i = 0; i < urls.length; i++) {
		...
		PytCmd('inserthtml', false, '<img src="' + $(urls[i]).attr('src') + '" alt="' + alt + '"' + w + h + ' />');
	}
	PytSH('.PytDiv.Image');
}
...
function InitPuyuetianEditor($id, $flashcode) {
	...
	var pytEditorTemplate = $('#PytEditorTemplate').html();
	$parent.append('<div id="PytEditorDriveDiv"></div>');
	if (pytEditorTemplate) {
		$('#PytEditorDriveDiv').html(pytEditorTemplate);
		initEditor();
	} else {
		$('#PytEditorDriveDiv').load(
			'app/puyuetianeditor/template/PytEditorHtml.html',
			function(r, s, x) {
				if (s == 'success') {
					initEditor();
				}
