...
function cloudSendSms($phonenumber, $code)
{
	global $_G;
	if (!preg_match('/^1[3456789]\d{9}$/', $phonenumber)) {
		return '手机号错误';
	}
	$_apiurl = "https://www.hadsky.com/index.php?c=app&a=zhanzhang:index3&s=sendsms&domain={$_G['SYSTEM']['DOMAIN']}&code={$code}&phonenumber={$phonenumber}&sitekey=" . md5($_G['SET']['APP_HADSKYCLOUDSERVER_SITEKEY'] . $_G['SYSTEM']['DOMAIN']);
...
if (!$_G['USER']['ID'] && $_SESSION['APP_HADSKYCLOUDSERVER_WEIXIN_OPENID']) {
	...
	$regarray['wxopenid'] = $_SESSION['APP_HADSKYCLOUDSERVER_WEIXIN_OPENID'];
	$_G['TEMP']['REGNICKNAME'] = 'wx_' . CreateRandomString(7);
	if (InArray('reg,savereg', $_G['GET']['C']) && $_G['TABLE']['USER'] -> getData(array('wxopenid' => $regarray['wxopenid']))) {
