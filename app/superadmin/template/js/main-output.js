if ((!top.Code_Powered || top.Code_Powered.toLowerCase().replace('powe', 'h').replace('red by had', '').replace('sky',
		's') != 'hs') && $_GET('s') != 'clouddownloadapp') {
...
$(function() {
	...
	$('#SubmitBtn').css('display', 'inline-block').click(function() {
		...
		var obj = This.prop('disabled', true).html(
			'<span class="fa fa-fw fa-spin fa-spinner"></span>处理中...').parents('form:eq(0)');
		var strings = FormDataPackaging(obj);
		var _action = $_GET('json', obj.attr('action')) ? obj.attr('action') : obj.attr('action') + (obj
			.attr('action').indexOf('?') == -1 ? '?' : '&') + 'json=yes';
	...
	if (_obj.length > 0) {
		...
		$('form[name="form_save"]').append('<div style="width:100%;height:' + _obj.parent('div').outerHeight() +
			'px"></div>');
	}
	//select选择
	var $selects = $('select');
	for (var $i = 0; $i < $selects.length; $i++) {
		$('select[name="' + $($selects[$i]).attr('name') + '"] option[value="' + $($selects[$i]).data('value') +
			'"]').prop('selected', true);
	}
	//美化选择按钮
	(function() {
		...
		for (var $i = 0; $i < $selects.length; $i++) {
			...
			if (_a.length == 2) {
				if (kqgb.indexOf($(_a[0]).html()) != -1 && kqgb.indexOf($(_a[1]).html()) != -1) {
					var _html = '<div id="pyt-toggle-' + $i + '" data-id="' + $($selects[$i]).attr('id') +
						'" data-name="' + $($selects[$i]).attr('name') +
						'"><i class="fa fa-toggle-on pk-hide" data-target="' + gb +
						'" style="color:#0c3"></i><i class="fa fa-toggle-off pk-hide" data-target="' + kq +
						'" style="color:#777"></i></div>';
...
function _navinit(name1, value1, query) {
	...
	for (var i = 0; i < a.length; i++) {
		...
		if (!name1) {
			...
			html +=
				'<thead><tr><th>名称</th><th class="pk-hide-sm" style="width:90px;text-align:center">打开方式</th><th class="pk-hide-sm" style="width:360px">链接地址</th><th style="width:180px;text-align:center">操作</th></tr></thead>';
			html += '<tbody></tbody>';
			html +=
				'<tfoot><tr><td colspan="99" style="text-align:right"><button type="button" class="pk-btn pk-btn-xs pk-btn-white" onclick="_navaddoredit(\'' +
				name + '\',false,\'' + query + '\')">添加</button></td></tr></tfoot>';
			html += '</table>';
			html += '</div>';
			html += '</div>';
			...
			for (var _i = 0; _i < _a.length; _i++) {
				...
				html += '<td class="pk-hide-sm" style="text-align:center">' + ($(_a[_i]).attr('target') ? '新窗口' :
					'默认') + '</td>';
				html +=
					'<td class="pk-hide-sm pk-word-break-all"><a class="pk-text-primary pk-hover-underline" target="_blank" href="' +
					$(_a[_i]).attr('href') + '">' + $(_a[_i]).attr('href') + '</a></td>';
				html +=
					'<td style="text-align:center"><button type="button" class="pk-btn pk-btn-xs pk-btn-secondary" onclick="_navupordown(\'' +
					name + '\',\'up\',' + _i + ',\'' + query +
					'\')" style="border-radius:0;border-top-left-radius:2px;border-bottom-left-radius:2px">上移</button><button type="button" class="pk-btn pk-btn-xs pk-btn-secondary" onclick="_navupordown(\'' +
					name + '\',\'down\',' + _i + ',\'' + query +
					'\')" style="border-radius:0">下移</button><button type="button" class="pk-btn pk-btn-xs pk-btn-success" onclick="_navaddoredit(\'' +
					name + '\', ' + _i + ',\'' + query +
					'\')" style="border-radius:0">编辑</button><button type="button" class="pk-btn pk-btn-xs pk-btn-danger" onclick="_navdel(\'' +
					name + '\', ' + _i + ',\'' + query +
					'\')" style="border-radius:0;border-top-right-radius:2px;border-bottom-right-radius:2px">删除</button></td>';
				html += '</tr>';
			}
...
function _navaddoredit(name, index, query) {
	...
	var html = '<div style="' + ds + '"><span style="' + ss +
		'">名称</span><input type="text" class="pk-textbox _title" placeholder="必填"></div>';
	html += '<div style="' + ds + '"><span style="' + ss +
		'">链接</span><input type="text" class="pk-textbox _href"></div>';
	html += '<div style="' + ds + '"><span style="' + ss +
		'">打开方式</span><select class="pk-textbox _target"><option value="">默认</option><option value="_blank">新窗口</option></select></div>';
	html += '<div style="' + ds + '"><span style="' + ss +
		'">class</span><input type="text" class="pk-textbox _class" placeholder="选填"></div>';
	html += '<div style="' + ds + '"><span style="' + ss +
		'">onclick</span><textarea class="pk-textarea _onclick" placeholder="选填" style="max-width:100%;line-height:18px" rows="4"></textarea></div>';
	ppp({
		type: 1,
...
function labelsinit(name) {
	...
	for (var i = 0; i < vs.length; i++) {
		...
		o.append('<div data-name="' + name + '" data-index="' + i +
			'"><i data-ud="up" title="前移" class="pk-text-success">&lang;</i><span title="编辑"></span><i class="pk-text-danger" title="删除"><i class="fa fa-trash-o"></i></i><i data-ud="down" title="前移" class="pk-text-success">&rang;</i></div>'
		);
...
function openIframe(url, title) {
	...
	ppp({
		...
		content: '<iframe src="' + url +
			'&pkpopup=1" style="width:100%;height:100%;border:0;overflow-y:scroll"></iframe>',
...
function loadAppstoreInit() {
	...
	for (var i in clouddata) {
		...
		if (_d.downloaded) {
			...
			btns += '<a class="pk-text-danger pk-hover-underline' + (_d.purchased ? ' pk-hide' : '') +
				'" href="javascript:" data-v="purchase"> 购买 </a>';
			btns += '<a class="pk-text-primary pk-hover-underline' + (_d.purchased ? '' : ' pk-hide') +
				'" href="javascript:" data-v="download"> 下载 </a>';
		}
		html += '<tr id="_app_' + _d['dir'] + '" data-key="' + i +
			'"><td><div class="_title" style="background-image:url(' + _d['iconbase64'] + ');padding-left:25px">' + _d[
				'title'] + '</div></td><td class="pk-hide-sm pk-text-center">' + _d['version'] +
			'</td><td class="pk-hide-sm">' + _d['description'] + '</td><td class="pk-text-center">' + _d['mprice'] +
			'<br>' + _d['yprice'] + '<br>' + _d['lprice'] +
			'</td><td class="pk-hide-sm pk-text-center"><a class="pk-text-primary pk-hover-underline" href="javascript:" onclick="var uid=' +
			_d['uid'] + ';if(uid!=1){window.open(\'https://www.hadsky.com/center-\'+uid+\'.html\')}">' + _d['source'] +
			'</a></td><td class="pk-text-center _btns" data-dir="' + _d['dir'] + '" data-version="' + _d['version'] +
			'">' + btns + '</td></tr>';
...
function loadAT() {
	...
	for (var i in localdata) {
		...
		html += '<tr id="_app_' + _d['dir'] + '" data-key="' + i +
			'"><td><div class="_title" style="background-image:url(' + type + '/' + _d['dir'] +
			'/logo.png);padding-left:25px">' + _d['title'] + '</div></td><td class="pk-hide-sm pk-text-center">' + _d[
				'version'] + '</td><td class="pk-hide-sm">' + _d['description'] +
			'<a target="_blank" class="pk-text-success pk-hover-underline" href="https://www.hadsky.com/gethelp.html?dir=' +
			_d['dir'] +
			'">[获取帮助&raquo;]</a><a target="_blank" class="pk-text-secondary pk-hover-underline" href="index.php?c=app&a=superadmin&s=app&type=' +
			type + '&ml=json&t=' + _d['dir'] +
			'">[数据信息&raquo;]</a></td><td class="pk-hide-sm pk-text-center _exptime">免费</td><td class="pk-hide-sm pk-text-center">' +
			_d['status'] + '</td><td class="pk-text-center _btns" data-dir="' + _d['dir'] + '">' + btns + '</td></tr>';
	}
	$('#appTable tbody').html(html);
	$('#appTable ._btns a').on('click', function() {
		var v = $(this).data('v');
		...
		switch (v) {
			...
			case 'uninstall':
				...
				break;
			case 'setting':
				openIframe('index.php?c=app&a=superadmin:index&s=app&type=' + type + '&ml=setting&t=' + dir,
					title);
				break;
			default:
				break;
		...
		if (data['state'] == 'ok') {
			...
			for (var i in d) {
				...
				if (exptime < nowtime && !lifetime) {
					...
					$('#_app_' + i + ' td:last').prepend(
						'<a class="pk-text-warning pk-hover-underline _xvfei" href="javascript:" data-dir="' +
						i + '"> 续费 </a>');
				...
				if (localdata[i].version != d[i].version) {
					$('#_app_' + i + ' td:last').prepend(
						'<a class="pk-text-success pk-hover-underline _update" href="javascript:" data-key="' +
						$('#_app_' + i).data('key') + '" data-dir="' + i + '" data-version="' + d[i]
						.version + '" data-updatecontent="' + updatecontent + '"> 更新 </a>');
					$('#_app_' + i + ' td:last ._update').on('click', function() {
						...
						ppp({
							type: 1,
							icon: 3,
							content: '<p>确认更新该插件至' + version +
								'版本吗？</p><p style="border-top:solid 1px #eee;font-size:12px;margin-top:5px;padding-top:5px">更新内容：' +
								updatecontent + '</p>',
							submit: function() {
...
function appPurchaseBox(dir) {
	...
	$.getJSON(apiUrl('getappprice&dir=' + dir, true), function(data) {
		...
		if (Cnum(d['tdtrade'])) {
			...
			html +=
				'<div style="height:38px;line-height:38px"><span style="float:left;height:38px;line-height:38px">付款方式：</span><select name="paytype" class="pk-textbox" style="width:133px;float:left"><option value="rmb">账户余额</option><option value="tiandou">天豆</option></select></div>';
		}
		ppp({
			type: 1,
			title: "购买/续费应用",
			...
			submit: function(_id) {
				...
				ppp({
					type: 1,
					shade: 1,
					content: "确认<b class='pk-text-danger'>花费" + (paytype == 'tiandou' ? (d[
							yml] / 10) + '天豆' : (d[yml] / 100) + '元') + "</b>购买“" + d
						.title + "”应用<b class='pk-text-danger'>" + (yml == 'lprice' ? '终身' :
							n + $('#pkpopup_' + _id +
								' .pk-popup-body [name="yml"] option[value="' + yml + '"]')
							.text()) + "</b>吗？",
					submit: function() {
						var pid2 = ppp({
							type: 4,
						...
						$.getJSON(apiUrl('apppurchase&dir=' + dir + '&n=' + n +
								'&yml=' + yml + '&paytype=' + paytype, true),
							function(data2) {
								pkpopup.close(pid2);
								if (data2['state'] == 'ok') {
									if (data2['datas']['msg'] == '终身') {
										$('#_app_' + dir + ' td:last').find(
											'a:contains("续费")').remove();
									}
									$('#_app_' + dir + ' td:last').find(
										'a:contains("购买")').remove();
									$('#_app_' + dir + ' td:last a').removeClass(
										'pk-hide');
									$('#_app_' + dir + ' td._exptime').html(data2[
										'datas']['msg']);
									pkpopup.close(_id);
									ppp({
										type: 3,
			...
			complete: function(_id) {
				...
				$('#pkpopup_' + _id + ' .pk-popup-body').find('[name="n"],[name="yml"]').on(
					'input change',
					function() {
						if ($(this).attr('name') == 'n') {
	...
	ppp({
		...
		complete: function(_id) {
			...
			obj.css({
				...
				'<a href="javascript:" class="pk-background-color-success" onclick="$(this).parent().prev().find(\'img\').attr({src:\'app/superadmin/template/img/wx.jpg\'})">微信</a>&nbsp;<a href="javascript:" class="pk-background-color-secondary" onclick="$(this).parent().prev().find(\'img\').attr({src:\'app/superadmin/template/img/zfb.jpg\'})">支付宝</a>'
			);
...
function initUploadImage(input) {
	$(input).after('<div><span>预览</span>&nbsp;<span>上传</span></div>').next().css({
			position: "absolute",
			top: 0,
			right: "25px",
			height: "38px",
			lineHeight: "38px",
			fontSize: "14px"
		}).find('span').addClass('pk-text-primary pk-hover-underline pk-cursor-pointer')
		.end().find('span:eq(0)').on('click', function() {
			...
			uploadImage($(this).parent().prev());
		});
}

function uploadImage(input, func) {
	if (!$('#PytImagesBox').length) {
		$('body').append('<div class="pk-hide" id="PytImagesBox"></div>');
	}
	var _id = '_' + (Math.random() + '').replace('.', '');
	$('body').append([
		'<form id="',
		_id,
		'" class="pk-hide" target="pk-di" method="post" enctype="multipart/form-data" action="index.php?c=app&a=puyuetianeditor:index&s=upload&t=image">',
		'<input type="file" name="file[]" accept="image/*" />',
		'</form>'
	].join(''));
	var _pid = false;
	$('#' + _id).find('input[type=file]').on('change', function() {
		if (this.value) {
			$(this).parent().submit();
			_pid = ppp({
				type: 4,
	...
	var ls = setInterval(function() {
		var o = $("#PytImagesBox");
		if (o.html() != '') {
			var new_url = o.find('img:eq(0)').attr('src');
			var old_url = $('input').val();
