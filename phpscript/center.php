...
switch ($_G['GET']['TYPE']) {
	case 'dynamic':
		...
		$query = sqlQuery("select `id`,`posttime`,`title` as `t_r`,'read' as `table` from `{$_G['MYSQL']['PREFIX']}read` where `uid`={$_G['TEMPUSER']['ID']}" . ($_G['SET']['HIGHSPEED'] ? '' : ' and `del`=0') . " union all select `id`,`posttime`,`rid` as `t_r`,'reply' as `table` from `{$_G['MYSQL']['PREFIX']}reply` where `uid`={$_G['TEMPUSER']['ID']}" . ($_G['SET']['HIGHSPEED'] ? '' : ' and `del`=0') . " order by `posttime` desc limit {$spos},{$num}");
		$array = array();
		$index = 0;
