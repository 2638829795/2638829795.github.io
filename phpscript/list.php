...
if ($sortid) {
	...
	if (!$_G['SET']['HIGHSPEED'] && ($_G['SET']['READLISTSHOWBKS'] || $_G['SET']['READLISTHIDDENBKS'])) {
...
for ($i = 0; $i < 2; $i++) {
	...
	if (!$sortid && !$i) {
		...
		if ($sortid) {
			...
			$_sql = '';
			if ($_G['SET']['ACTIVETOPREADIDS'] && !$_G['SET']['HIGHSPEED']) {
				$_aids = explode(',', $_G['SET']['ACTIVETOPREADIDS']);
				foreach ($_aids as $_id) {
		...
		if ($_G['SET']['HIGHSPEED']) {
			if (ltrim($sql2, ' and ')) {
				$hs_sql = 'where ' . ltrim($sql2, ' and ') . ' order by `' . Cstr($_G['SET']['READLISTORDER'], 'activetime', true, 1, 255) . '` desc';
			} else {
	...
	if ($i) {
		if ($_G['SET']['HIGHSPEED']) {
			...
			$readcount = $_G['TABLE']['READ'] -> getCount('where `del`=0' . $sql2);
		}
